...
class MockEnergyMonitor:
    def __init__(self):
        # Simulate some default GPU and CPU indices.
        self.gpu_indices = [0, 1, 2, 3]
        self.cpu_indices = [0, 1]
        self.dram_indices = [0, 1]

    def begin_window(self, name):
        print(f"Mock begin window: {name}")

    def end_window(self, name):
        print(f"Mock end window: {name}")
        # Simulate the end_window response with predefined energy values.
        return MockMeasurement(
            gpu_energy={index: random.uniform(50.0, 500.0) for index in self.gpu_indices},  # Random values between 50 and 500
            cpu_energy={index: random.uniform(10.0, 200.0) for index in self.cpu_indices},  # Random values between 10 and 200
            dram_energy={index: random.uniform(5.0, 100.0) for index in self.dram_indices}   # Random values between 5 and 100
        )
...
class MockPowerMonitor:
    ...
    def begin_window(self, name):
        print(f"Mock power monitor begin window: {name}")

    def get_power(self):
        # Simulate power measurements for GPUs.
        print(f"Mock power monitor getting power meaasurements")
        return [(index, random.uniform(50.0, 500.0)) for index in self.gpu_indices]

    def end_window(self, name):
        print(f"Mock power monitor end window: {name}")

if __name__ == '__main__':

    mock_energy_monitor = MockEnergyMonitor()
    mock_power_monitor = MockPowerMonitor()

    histogram_metric = EnergyHistogram(energy_monitor=mock_energy_monitor, prometheus_url='http://localhost:9091', job='mock_energy_monitoring')
    cumulative_counter_metric = EnergyCumulativeCounter(energy_monitor=mock_energy_monitor, update_period=2, prometheus_url='http://localhost:9091', job='mock_energy_counter_monitoring')
    power_gauge_metric = PowerGauge(power_monitor=mock_power_monitor, update_period=2, prometheus_url='http://localhost:9091', job='mock_power_monitoring')

    # Testing the Counter metrics
    ...
    #Testing the Histogram metrics
    print("Testing Histogram Metric")
    histogram_metric.begin_window("Histogram_test_window")
    # When end_window gets called, the function generates random values for each indices for each devices
    histogram_metric.end_window("Histogram_test_window")
