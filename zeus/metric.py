...
class EnergyHistogram(Metric):
    ...
    def __init__(
        ...
        self.bucket_ranges["gpu"] = (
            bucket_ranges.get("gpu")
            if bucket_ranges and "gpu" in bucket_ranges
            else default_gpu_buckets
        )

        self.bucket_ranges["cpu"] = (
            bucket_ranges.get("cpu")
            if bucket_ranges and "cpu" in bucket_ranges
            else default_cpu_buckets
        )

        self.bucket_ranges["dram"] = (
            bucket_ranges.get("dram")
            if bucket_ranges and "dram" in bucket_ranges
            ...
            for gpu_index, gpu_energy in measurement.gpu_energy.items():
                if gpu_index in self.gpu_histograms:
                    self.gpu_histograms[gpu_index].labels(
                        window=f"__EnergyHistogram_{name}", index=gpu_index
                    ).observe(gpu_energy)
            ...
            for cpu_index, cpu_energy in measurement.cpu_energy.items():
                if cpu_index in self.cpu_histograms:
                    self.cpu_histograms[cpu_index].labels(
                        window=f"__EnergyHistogram_{name}", index=cpu_index
                    ).observe(cpu_energy)
            ...
            for dram_index, dram_energy in measurement.dram_energy.items():
                if dram_index in self.dram_histograms:
                    self.dram_histograms[dram_index].labels(
                        window=f"__EnergyHistogram_{name}", index=dram_index
                    ).observe(dram_energy)
            ...
            for gpu_index, energy in measurement.gpu_energy.items():
                if gpu_index in gpu_counters:
                    gpu_counters[gpu_index].labels(
                        window=f"__EnergyCumulativeCounter_{name}", index=gpu_index
                    ).inc(energy)
            ...
            for cpu_index, energy in measurement.cpu_energy.items():
                if cpu_index in cpu_counters:
                    cpu_counters[cpu_index].labels(
                        window=f"__EnergyCumulativeCounter_{name}", index=cpu_index
                    ).inc(energy)
            ...
            for dram_index, energy in measurement.dram_energy.items():
                if dram_index in dram_counters:
                    dram_counters[dram_index].labels(
                        window=f"__EnergyCumulativeCounter_{name}", index=dram_index
                    ).inc(energy)
