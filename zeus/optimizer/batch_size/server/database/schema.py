...
class Job(Base):
    ...
    def populate_from_job_spec(self, jobSpec: JobSpec):
        ...
        self.mab_prior_mean = jobSpec.mab_prior_mean
        self.mab_prior_precision = jobSpec.mab_prior_precision
        self.mab_seed = jobSpec.mab_seed
        self.mab_num_exploration = jobSpec.mab_num_exploration
        if self.mab_seed != None:
            rng = np.random.default_rng(jobSpec.mab_seed)
...
class ExplorationState(Base):
    ...
    state: Mapped[ExplorationOfBs.State] = mapped_column(
        Enum(ExplorationOfBs.State), default=ExplorationOfBs.State.Exploring
    )
