...
class carbon_monitor:
    def __init__(self, interval_min=30, start_step=48):
        ...
        self.df['sin_time'] = np.sin(2*np.pi*self.df.time_enc/self.total_steps_day)
        self.df['cos_time'] = np.cos(2*np.pi*self.df.time_enc/self.total_steps_day)
        self.df['prev'] = get_prev(self.df['hist'].values, 1)
        X = self.df[['sin_time', 'cos_time', 'prev']]
        Y = self.df[['hist']]
        X_train, y_train = X.values[start_step-48:start_step], Y.values.flatten()[start_step-48:start_step]
    ...
    def get_forecast(self):
        t = self.step_counter
        input_ = [self.df.iloc[t]['sin_time'], self.df.iloc[t]['cos_time'], self.df.iloc[t]['prev']]
        return self.model.predict([input_])[0]

def carbon_cost(power, tput, power_limits, next_carbon_intensity, max_carbon=750, max_pl=300, eta_knob=0.24, world_size=1, return_feature=False):
...
def recur_hist_forecasting(model, X_test, y_test, skip_step=1):
    ...
    for i in range(steps):
        ...
        if skip_step=="random" and np.random.randint(0,2) and i!=0:
            if len(X_test[0])>3: last_val = X_test[i][2:]
            else: last_val = X_test[i][-1]
