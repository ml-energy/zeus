...
class OpenEIClient(ElectricityPriceProvider):
    ...
    def search_json(self, data, key_name, target_value, return_value):
        """Recursively search for a key in a nested JSON and return the return_value if found."""
        results = []
...
def main():
    try:
        ...
        print(
            f"Location fetched successfully: Latitude = {location[0]}, Longitude = {location[1]}"
        )
    except Exception as e:
        print(f"Could not get location: {e}")
        return

    label = "5b8473395457a3004cedfe02"
    ...
    try:
        ...
        search_results = client.search_json(
            sample_json, "label", label, "energyratestructure"
        )
