...
class Jetson(SoC):
    ...
    def _discover_available_metrics(self) -> DeviceMap:
        ...
        """
        # The driver may be registered as "ina3221x" or "ina3221" depending on the kernel.
        ...
        for device in path.glob("*"):
            for subdevice in device.glob("*"):
                # Rail files may live directly in the subdevice dir or one level deeper under hwmon*.
                scan_dirs = [subdevice]
                if subdevice.is_dir():
