...
Currently, metric names (e.g., `energy_monitor_gpu_energy_joules`) are static and cannot be overridden. However, users can customize the name of the window to define the context of the metrics.

- **Metric Name**: `energy_monitor_gpu_energy_joules`
- **Labels**:
...
### 1. Query Metrics Examples
Retrieve current power consumption for GPU index 0:
```promql
power_monitor_gpu_power_watts{index="0"}
```
Retrieve current power consumption for specific window:
```promql
power_monitor_gpu_power_watts{window="defined_window_name"}
```
Retrieve average GPU power consumption over the last 5 minutes
```promql
avg_over_time(power_monitor_gpu_power_watts[5m])
```
...
if __name__ == "__main__":
    ...
    for epoch in range(100):
        ...
        energy_histogram.begin_window(f"epoch_energy")
        ...
        energy_histogram.end_window(f"epoch_energy")
